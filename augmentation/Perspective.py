...
def rand_shift(images, ratio=0.125):
    ...
    images = tf.gather_nd(tf.pad(images, [[0, 0], [1, 1], [0, 0], [0, 0]]), tf.expand_dims(grid_x, -1), batch_dims=1)
    images = tf.transpose(tf.gather_nd(tf.pad(tf.transpose(images, [0, 2, 1, 3]), [[0, 0], [1, 1], [0, 0], [0, 0]]), tf.expand_dims(grid_y, -1), batch_dims=1), [0, 2, 1, 3])
    return tf.slice(images, [0, pad_size, pad_size, 0], [-1, src_height, src_width, -1])